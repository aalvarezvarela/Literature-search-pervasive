...
class PubMedArticle(object):
    ...
    def _get_publication_types(self, xml_element):
        outd = {}
        pubtypes = xml_element.findall('MedlineCitation/Article/PublicationTypeList/PublicationType')
        for pt in pubtypes:
            outd[pt.get('UI')] = pt.text
        return outd
    def _get_grantlist(self,xml_element):
        outl = []
        grants = xml_element.findall('MedlineCitation/GrantList')
        for gr in grants:
            outl.append({'agency': gr.get('Agency', None), 'country': gr.get('Country', None)})
        return outl
    def _get_mesh_headings(self,xml_element):
        meshtags = xml_element.findall('MedlineCitation/MeshHeadingList/MeshHeading')
        outd = {}
    ...
    def _initializeFromXML(self: object, xml_element: TypeVar("Element")) -> None:
        ...
        self.publication_types = self._get_publication_types(xml_element)
        self.grants = self._get_grantlist(xml_element)
        self.mesh_terms = self._get_mesh_headings(xml_element)
